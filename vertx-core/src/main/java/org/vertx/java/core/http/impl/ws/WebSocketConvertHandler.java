...
public class WebSocketConvertHandler extends MessageToMessageCodec<io.netty.handler.codec.http.websocketx.WebSocketFrame, WebSocketFrame> {
  ...
  protected void decode(ChannelHandlerContext ctx, io.netty.handler.codec.http.websocketx.WebSocketFrame msg, MessageBuf<Object> out) throws Exception {
    if (msg instanceof BinaryWebSocketFrame) {
      out.add(new DefaultWebSocketFrame(WebSocketFrame.FrameType.BINARY, msg.data().copy()));
      return;
    }
    if (msg instanceof CloseWebSocketFrame) {
      out.add(new DefaultWebSocketFrame(WebSocketFrame.FrameType.CLOSE, msg.data().copy()));
      return;
    }
    if (msg instanceof PingWebSocketFrame) {
      out.add(new DefaultWebSocketFrame(WebSocketFrame.FrameType.PING, msg.data().copy()));
      return;
    }
    if (msg instanceof PongWebSocketFrame) {
      out.add(new DefaultWebSocketFrame(WebSocketFrame.FrameType.PONG, msg.data().copy()));
      return;
    }
    if (msg instanceof TextWebSocketFrame) {
      out.add(new DefaultWebSocketFrame(WebSocketFrame.FrameType.TEXT, msg.data().copy()));
      return;
    }
    if (msg instanceof ContinuationWebSocketFrame) {
      out.add(new DefaultWebSocketFrame(WebSocketFrame.FrameType.CONTINUATION, msg.data().copy()));
      return;
    }
    throw new IllegalStateException("Unsupported websocket msg " + msg);
  }
